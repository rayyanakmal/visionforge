...
- Upload demo kit: `examples/preds_v2_mixed.json` + `examples/preds_v2_clean.json`,
  generated by `data/generate_upload_demo.py` from the REAL YOLOv8n run-A output
  (the Pi never runs a model — D5). Two stories the built-in sample doesn't show:
  - `v2_mixed`: person REGRESSED (−0.146) while dining table IMPROVED (+0.743) —
    aggregate mAP actually rises 0.315 → 0.338 as a class breaks ("mAP hides it").
  - `v2_clean`: only additions → verdict PASS, clean-release banner.

### Changed

- Report card tab now shows BOTH runs as nested tabs when two are loaded
  (previously only run A; run B's card was buried in Compare's nested tabs).
- Compare tab no longer repeats the full report cards (they live in the Report
  card tab) — Compare keeps verdict banner, aggregate side-by-side, delta table.
